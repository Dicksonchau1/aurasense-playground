...
const TIPS: Record<keyof LaneScores, string[]> = {
  posture: ['Keep your shoulders level and back straight.', 'Avoid leaning to one side during your response.', 'A balanced posture signals composed self-presentation.'],
  gaze: ['Look directly at the camera lens, not the screen.', 'Reduce side-glancing by placing notes near the webcam.', 'Consistent gaze direction improves coherence perception.'],
  framing: ['Position your face in the upper-center third of the frame.', 'Ensure even headroom above your head.', 'Move closer or further until the score improves.'],
  pacing: ['Allow brief pauses between thoughts — silence is practice.', 'Aim to speak about 60–70% of the session duration.', 'Vary sentence rhythm to maintain coherent delivery.'],
}
    ...
    <div className="flex flex-col h-full p-4">
      <div className="flex gap-1 mb-4 p-1 rounded-lg" style={{ background: 'var(--bg)' }}>
        {['Envelope', 'Lanes', 'Tips'].map(tab => (
          <button key={tab} className="flex-1 text-xs py-1.5 rounded-md transition-colors" style={{
      ...
      <div className="rounded-xl p-5 mb-4 border" style={{ background: 'var(--bg)', borderColor: 'var(--border)' }}>
        ...
        <div className="flex items-end gap-2 mb-2">
          <span aria-live="polite" className="text-6xl font-bold tabular-nums" style={{ color: isActive ? 'var(--text)' : 'var(--muted)' }}>{envelope}</span>
