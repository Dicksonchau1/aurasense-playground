...
export function MembershipDrawer({ open, onClose }: MembershipDrawerProps) {
  ...
  return (
    <Drawer open={open} onOpenChange={o => !o && onClose()}>
      <DrawerContent style={{ background: 'var(--panel)', borderColor: 'var(--border)' }}>
        <DrawerHeader className="border-b pb-4" style={{ borderColor: 'var(--border)' }}>
          ...
          <DrawerDescription style={{ color: 'var(--muted)' }}>Unlock the full practice environment</DrawerDescription>
        </DrawerHeader>
        <div className="p-6 space-y-6">
          ...
          <a href="mailto:hello@aurasensehk.com?subject=Rehearse%20Pro%20trial" className="block w-full py-3 rounded-xl text-center text-sm font-semibold hover:opacity-90" style={{ background: 'var(--accent-green)', color: '#000' }}>
            Start free 14-day trial
          </a>
          <a href="mailto:hello@aurasensehk.com?subject=Rehearse%20Pro%20trial" className="block w-full py-3 rounded-xl text-center text-sm font-medium border hover:opacity-80" style={{ borderColor: 'var(--border)', color: 'var(--text)' }}>
