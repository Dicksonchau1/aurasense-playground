...
export function RegulatoryComplianceSection({
  ...
  export function RegulatoryComplianceSection({
    ...
    const effectiveSelectedControl = selectedControlId
      ? state.controls.find((c: any) => c.id === selectedControlId) || selectedControl
      : selectedControl;

    const vm = getRegulatoryCompliancePanelVM(state, effectiveSelectedControl);

    return (
      <section className={className} aria-label="Regulatory Compliance Section">
          ...
          <div className="flex flex-col gap-4 border-b border-white/10 pb-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              {!hideActions && (
                <button
                  type="button"
                  aria-label="Sign compliance export"
                  onClick={onSignExport || signComplianceExport}
            ...
            <div className="space-y-3">
              {vm.rows.map((row) => (
                <button
                  key={row.id}
                  type="button"
                  aria-label={`Select control ${row.label}`}
                  onClick={() => (onSelectControl ? onSelectControl(row.id) : selectControl(row.id))}
