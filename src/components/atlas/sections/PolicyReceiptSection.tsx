...
export interface PolicyReceiptPanelVM {
...
const PolicyReceiptSection: React.FC<Props> = ({
  vm,
  onAcknowledge,
  onDownloadPolicy,
}) => {
  return (
    <div>
      ...
      <div className={styles.marginTop16}>
        <button onClick={() => onAcknowledge("Operator")}>Acknowledge Policy</button>
        {vm.policyDocUrl && (
          <a href={vm.policyDocUrl} target="_blank" rel="noopener noreferrer">
            <button onClick={onDownloadPolicy}>Download Policy</button>
          </a>
        )}
      </div>
