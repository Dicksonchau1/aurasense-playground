...
export function ExternalDisclosureSection({
  ...
  const vm = getExternalDisclosurePanelVM(state, selectedDisclosure);
          ...
          <div className="space-y-3">
            {vm.rows.map((row) => (
              <button
                key={row.id}
                type="button"
                onClick={() => selectDisclosure(row.id)}
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {(["draft", "review", "ready", "submitted", "blocked"] as const).map((status) => (
                    <button
                      key={status}
                      type="button"
                      onClick={() => setDisclosureStatus(vm.selectedDisclosure!.id, status)}
