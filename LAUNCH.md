...
- Pacing score uses microphone input but does not yet do voice activity detection — RMS threshold may misclassify background noise as speech
...
- OG image uses static /og.png fallback on some crawlers that don't execute JS
- Lighthouse performance may dip below 85 on first paint if MediaPipe WASM takes >1s to load
